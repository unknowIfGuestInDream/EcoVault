...
public class PasswordServiceImpl implements PasswordService {
    ...
    public List<PasswordEntryResponse> list(Long userId, String keyword, String tag) {
        ...
        List<PasswordEntryResponse> responses = entries.stream()
                .map(this::toResponse)
    ...
    private PasswordEntryResponse toResponse(PasswordEntry entry) {
        return new PasswordEntryResponse(
                ...
                entry.getTitle(),
                entry.getAccount(),
                aesUtil.decrypt(entry.getSecret()),
                entry.getUrl(),
                aesUtil.decrypt(entry.getNotes()),
                entry.getCategory(),
                decryptTags(entry.getTags()),
                entry.getStrengthScore(),
                entry.getStrengthLevel(),
                entry.getCreatedAt(),
                entry.getUpdatedAt()
        );
    }
