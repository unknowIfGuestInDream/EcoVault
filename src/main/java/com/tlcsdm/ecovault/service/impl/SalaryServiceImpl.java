...
public class SalaryServiceImpl implements SalaryService {
    ...
    public List<SalaryResponse> list(Long userId, Integer year) {
        List<SalaryRecord> records = year == null
                ? repository.findByUserIdOrderByYearAscMonthAsc(userId)
    ...
    public SalaryStatistics statistics(Long userId, Integer year) {
        ...
        List<SalaryStatistics.MonthlyPoint> trend = records.stream()
                .map(r -> new SalaryStatistics.MonthlyPoint(
                        String.format("%04d-%02d", r.getYear(), r.getMonth()),
                        r.getNet(),
                        r.getGross()))
    ...
    public String exportCsv(Long userId, Integer year) {
        ...
        for (SalaryRecord r : records) {
            sb.append(r.getYear()).append(',')
                    .append(r.getMonth()).append(',')
                    .append(nz(r.getBaseSalary())).append(',')
                    .append(nz(r.getBonus())).append(',')
                    .append(nz(r.getAllowance())).append(',')
                    .append(nz(r.getDeduction())).append(',')
                    .append(r.getGross()).append(',')
                    .append(r.getNet()).append(',')
