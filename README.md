...
- favicon 固定放置在 `src/main/resources/static/favicon.ico`，并同步打包到 `src/main/resources/META-INF/resources/favicon.ico` 作为 Jar 兼容回退资源；页面统一通过 `/favicon.ico` 引用。应用额外提供显式的 `/favicon.ico` MVC 映射并返回 `image/x-icon`，同时按 Spring Boot 标准静态资源目录顺序（`/META-INF/resources`、`/resources`、`/static`、`/public`）回退查找，避免 fat jar 运行或反向代理场景下资源定位过于依赖单一路径。
