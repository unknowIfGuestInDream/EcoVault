...
设计要求：支持暗色/亮色主题切换，采用玻璃拟态卡片、渐变背景、圆角阴影与响应式布局。公共 `head` 片段统一注入 `favicon.ico`，`<link>` 标签使用 `type="image/x-icon"` 明确 MIME 类型（不设置 `sizes="any"`，该属性仅适用于 SVG 可缩放图标），确保所有主流浏览器均能正确识别并展示图标；图标文件固定放置于 `src/main/resources/static/favicon.ico`，页面统一通过 `/favicon.ico` 引用，打包后位于 `BOOT-INF/classes/static/favicon.ico`。除默认静态资源映射外，`PageController` 还提供显式 `/favicon.ico` MVC 映射并返回 `image/x-icon` 与公开缓存头，降低 fat jar / 反向代理场景下的资源定位差异；同时通过 `SecurityConfig` 的 `authorizeHttpRequests.permitAll` 开放 `/favicon.ico` 路径，避免浏览器自动请求时被重定向至登录页。若部署端使用 Nginx 且包含额外 `location` 片段（如 `include /etc/nginx/default.d/*.conf`），必须确认 `/favicon.ico` 仍会转发到应用，而不是由 Nginx 本地文件系统直接处理。此外统一输出 `<meta name="renderer" content="webkit">` 与 `<meta name="google" content="notranslate"/>`。后台首页的 Actuator 端点概览通过 `/api/admin/actuator-endpoints` 返回基础端点，仅展示可直接访问的非模板端点，避免将 `/actuator/env/{toMatch}`、`/actuator/loggers/{name}`、`/actuator/health/{*path}`、`/actuator/metrics/{requiredMetricName}` 之类模板路径直接渲染为链接。工资趋势、年度统计、收入构成、收支标签/月度趋势与后台状态图表使用 Chart.js。
