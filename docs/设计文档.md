...
隐私模式：登录后页面导航栏提供「🕶️ 隐私模式」一键切换按钮，点击后立即以覆盖全屏的毛玻璃遮罩隐藏当前页面全部内容（并对底层内容额外施加模糊与禁用交互），防止旁人窥视。遮罩状态通过 `localStorage` 的 `ecovault-privacy` 键持久化，刷新或页面跳转后仍保持锁定；解锁需在遮罩内输入当前账户登录密码，前端调用 `POST /api/auth/verify-password` 校验，校验通过才移除遮罩。该接口仅比对密码，不重新签发 JWT、不刷新会话，也不延长令牌有效期，与令牌 2 小时过期后的刷新逻辑相互独立。为避免匿名场景下的死锁，隐私模式仅在存在导航栏的登录后页面生效。
