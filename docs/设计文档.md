...
职责：密码条目增删改查、标签管理、搜索、AES 加密存储，以及列表脱敏展示、详情查看与安全删除确认。

主要类：`PasswordController`、`PasswordService`、`PasswordStrengthUtil`、`AesUtil`、`PasswordEntryRepository`。

流程：用户提交条目，后端校验权限与字段，敏感字段 AES-GCM 加密后保存，AOP 记录操作日志；列表查询默认返回脱敏密码，前端查看详情/显示密码/复制密码时再按条目单独拉取明文详情，删除当前详情条目后同步关闭详情弹窗并清理缓存，减少敏感信息在列表态暴露。
...
1. 工资：用户按工资条分类录入工资记录，系统校验年月与金额后保存，按月份、年份汇总并通过 Chart.js 展示；录入/编辑弹窗右上角提供关闭按钮，便于键盘与鼠标快速退出。工资条按以下分类录入并派生统计：
   ...
   - **到账信息**：按 **税前工资 → 所得税 → 税后工资 → 大病医疗 → 采暖补贴 → 实发金额** 的顺序展示；其中税前/税后工资为派生值，**实发金额** 独立录入，表示本人银行卡实际到账金额。
   ...
   - 自动化测试覆盖显式 `net_pay` 的录入、列表返回、统计、导出与非兼容旧回填逻辑，避免“计算口径”与“到账口径”在回归时混淆。
...
其中角色管理基于 `MenuPage` 枚举维护页面元数据（key、名称、路径、分组、是否仅管理员、是否可配置），管理员可在角色管理页为普通角色分配「可配置页面」（密码管理、工资管理、收入支出管理）的访问权限，权限持久化于 `role_permissions` 表。`ADMIN` 角色固定拥有全部页面访问权，服务端禁止通过 API 修改其配置。后台管理分组页面（用户管理、日志管理、角色管理）固定仅管理员可访问；控制台与个人中心对所有登录用户开放。创建用户时可为其分配角色。后台首页展示构建时间、Spring Boot/Java 运行信息、健康状态和当前已暴露的 Actuator 端点入口，便于运维排查。
...
- 隐私模式解锁通过 `POST /api/auth/verify-password` 仅校验当前登录密码，不签发新令牌、不刷新会话、不延长令牌有效期，与上述 2 小时令牌过期刷新机制相互独立。
...
| 用户 | POST | `/api/auth/verify-password` | 校验当前登录密码（隐私模式解锁，不刷新令牌/会话） | USER |
...
| 管理 | GET | `/api/admin/build-info` | 构建信息（含名称、版本、构建时间、Java/编码/激活配置） | ADMIN |
| 管理 | GET | `/api/admin/actuator-endpoints` | 返回可直接访问的 Actuator 基础端点列表 | ADMIN |

## 7. 前端设计

页面：`/`、`/login`、`/dashboard`、`/passwords`、`/finance`（工资管理）、`/finance/ledger`（收入支出管理）、`/profile`、`/admin`（后台首页：构建信息/系统状态/Actuator 端点概览）、`/admin/users`（用户管理）、`/admin/logs`（日志管理）、`/admin/roles`（角色管理）、`/error`。

导航采用下拉子菜单：财务管理分组包含工资管理与收入支出管理；后台管理分组（仅管理员可见）点击父菜单进入 `/admin` 后台首页，下拉菜单仅保留用户管理、日志管理与角色管理。前端根据 `/api/auth/me` 返回的 `pages` 可访问页面列表动态渲染菜单项。密码管理页采用表格展示，密码列默认显示 `******`，脱敏文案以前后端统一常量输出，并在同一行提供显示密码、复制、详情、编辑、删除操作；新增/编辑弹窗与详情弹窗右上角提供关闭按钮，删除操作统一使用支持 Escape / Enter / Tab 键盘交互的站内确认弹窗。工资录入弹窗右上角同样提供关闭按钮。日志管理筛选区采用一体化时间范围输入，并提供清空搜索条件按钮。
...
隐私模式：登录后页面导航栏提供「🕶️ 隐私模式」一键切换按钮，点击后立即以覆盖全屏的毛玻璃遮罩隐藏当前页面全部内容（并对底层内容额外施加模糊与禁用交互），防止旁人窥视。遮罩状态通过 `localStorage` 的 `ecovault-privacy` 键持久化，刷新或页面跳转后仍保持锁定；解锁需在遮罩内输入当前账户登录密码，前端调用 `POST /api/auth/verify-password` 校验，校验通过才移除遮罩。该接口仅比对密码，不重新签发 JWT、不刷新会话，也不延长令牌有效期，与令牌 2 小时过期后的刷新逻辑相互独立。为降低公共资源复杂度，隐私模式脚本与样式拆分为独立的 `privacy.js`、`privacy.css`。为避免匿名场景下的死锁，隐私模式仅在存在导航栏的登录后页面生效。

设计要求：支持暗色/亮色主题切换，采用玻璃拟态卡片、渐变背景、圆角阴影与响应式布局。公共 `head` 片段统一注入 `favicon.ico`，`<link>` 标签使用 `type="image/x-icon"` 明确 MIME 类型（不设置 `sizes="any"`，该属性仅适用于 SVG 可缩放图标），确保所有主流浏览器均能正确识别并展示图标；图标文件固定放置于 `src/main/resources/static/favicon.ico`，并同步打包到 `src/main/resources/META-INF/resources/favicon.ico` 作为 Jar 兼容回退资源，页面统一通过 `/favicon.ico` 引用。除默认静态资源映射外，`PageController` 还提供显式 `/favicon.ico` MVC 映射并返回 `image/x-icon` 与公开缓存头，且会按 Spring Boot 标准静态资源目录顺序（`/META-INF/resources`、`/resources`、`/static`、`/public`）回退查找，降低 fat jar / 反向代理场景下资源定位差异；同时通过 `SecurityConfig` 的 `authorizeHttpRequests.permitAll` 开放 `/favicon.ico` 路径，避免浏览器自动请求时被重定向至登录页。若部署端使用 Nginx 且包含额外 `location` 片段（如 `include /etc/nginx/default.d/*.conf`），必须确认 `/favicon.ico` 仍会转发到应用，而不是由 Nginx 本地文件系统直接处理。此外统一输出 `<meta name="renderer" content="webkit">` 与 `<meta name="google" content="notranslate"/>`。后台首页的 Actuator 端点概览通过 `/api/admin/actuator-endpoints` 返回基础端点，仅展示可直接访问的非模板端点，避免将 `/actuator/env/{toMatch}`、`/actuator/loggers/{name}`、`/actuator/health/{*path}`、`/actuator/metrics/{requiredMetricName}` 之类模板路径直接渲染为链接。工资趋势、年度统计、收入构成、收支标签/月度趋势与后台状态图表使用 Chart.js。
...
- **进程检测**：通过 `ps -ef | grep "java .*<APP_JAR>"` 查询进程是否运行，不再以 `ecovault.pid` 文件作为唯一判断依据。PID 文件仅用于辅助记录，启动/停止/健康检查均优先使用 `ps -ef` 结果，避免旧 PID 文件残留导致误判。
- 生产环境默认 JVM 参数包含 `-Xms128m -Xmx512m`、`--enable-native-access=ALL-UNNAMED` 与 UTF-8 编码选项；若部署机资源或访问量不同，可通过 `JAVA_OPTS` 覆盖。当前空闲时内存主要来自 JVM 堆、Spring Boot/Tomcat 线程、Hibernate 元数据与类加载缓存，若使用 `ps` 看到较高 `%CPU`，需先区分是否仍处于启动/JIT 预热阶段，再结合 `top -p <pid>` 观察瞬时值。
...
- 应用日志级别默认使用 `INFO`；如需临时排查问题，可通过环境变量 `ECOVAULT_APP_LOG_LEVEL=DEBUG` 覆盖，排查结束后应恢复为较低级别以减少空闲日志开销。
