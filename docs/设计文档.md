...
- JWT 仍采用 HttpOnly Cookie 传输，但由于服务端已启用 `CookieCsrfTokenRepository`，因此浏览器写操作需要同时回传 `XSRF-TOKEN` 对应的 `X-XSRF-TOKEN` 头，JWT Cookie 并未绕过 CSRF 防护。
- `/api/**` 统一启用 CORS 白名单，并要求请求携带 `Origin` 头；仅允许本地开发来源与 `ecovault.security.request.allowed-origins` 中声明的受信任域名访问，公网域名默认通过 `ECOVAULT_PUBLIC_ORIGIN` 注入。
- `/api/**` 统一校验 `User-Agent`，缺失、命中常见爬虫标识或浏览器主版本低于配置下限时直接拒绝请求。
- 页面输出统一 HTML 转义。
