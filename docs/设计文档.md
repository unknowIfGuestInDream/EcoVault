...
1. 工资：用户录入工资记录，系统校验金额和日期，保存后按月份、年份汇总，并通过 Chart.js 展示。
...
其中角色管理基于 `MenuPage` 枚举维护页面元数据（key、名称、路径、分组、是否仅管理员、是否可配置），管理员可在角色管理页为各角色（`USER`/`ADMIN`）分配「可配置页面」（密码管理、工资管理、收入支出管理）的访问权限，权限持久化于 `role_permissions` 表。后台管理分组页面（用户管理、日志管理、角色管理）固定仅管理员可访问；控制台与个人中心对所有登录用户开放。创建用户时可为其分配角色。
...
| 管理 | PUT | `/api/admin/roles/{role}/permissions` | 更新角色页面权限 | ADMIN |
| 管理 | GET | `/api/admin/build-info` | 构建信息 | ADMIN |

## 7. 前端设计

页面：`/`、`/login`、`/dashboard`、`/passwords`、`/finance`（工资管理）、`/finance/ledger`（收入支出管理）、`/profile`、`/admin`（用户管理）、`/admin/logs`（日志管理）、`/admin/roles`（角色管理）、`/error`。

导航采用下拉子菜单：财务管理分组包含工资管理与收入支出管理；后台管理分组（仅管理员可见）包含用户管理、日志管理与角色管理。前端根据 `/api/auth/me` 返回的 `pages` 可访问页面列表动态渲染菜单项。
...
- 收入支出记账（多标签、按标签/时间过滤、统计与 CSV 导出）、角色页面权限（矩阵、更新、越权判定）、日志管理（详情、时间区间、修改删除、仅管理员可见）与用户编辑删除均需覆盖。
