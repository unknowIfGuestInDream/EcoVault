...
职责：密码条目增删改查、标签管理、搜索、AES 加密存储，以及列表脱敏展示与详情查看。

主要类：`PasswordController`、`PasswordService`、`PasswordStrengthUtil`、`AesUtil`、`PasswordEntryRepository`。

流程：用户提交条目，后端校验权限与字段，敏感字段 AES-GCM 加密后保存，AOP 记录操作日志；列表查询默认返回脱敏密码，前端查看详情/显示密码/复制密码时再按条目单独拉取明文详情，减少敏感信息在列表态暴露。
...
导航采用下拉子菜单：财务管理分组包含工资管理与收入支出管理；后台管理分组（仅管理员可见）点击父菜单进入 `/admin` 后台首页，下拉菜单仅保留用户管理、日志管理与角色管理。前端根据 `/api/auth/me` 返回的 `pages` 可访问页面列表动态渲染菜单项。密码管理页采用表格展示，密码列默认显示 `******`，并在同一行提供显示密码、复制、详情、编辑、删除操作；新增/编辑弹窗与详情弹窗右上角提供关闭按钮，删除操作统一使用站内确认弹窗。日志管理筛选区采用一体化时间范围输入，并提供清空搜索条件按钮。
